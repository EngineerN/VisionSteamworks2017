 ...
 */
class GearPipelineThread {
private:
  grip::GearGripPipeline pipeline; //!< Interface to pipeline code
  ...
  void run() {
    for(int i = 0;; i++) {
      ...
      for(auto& contour : *contours) {
        ...
        contour_x_total += centerx + m_pixel_x_offset;
        contour_y_total += centery + m_pixel_y_offset;
        contour_count++;
      }
      if(contour_count) {
...
public:
  ...
  */
  void start() { m_thread = std::thread(&GearPipelineThread::run, this); }
};
